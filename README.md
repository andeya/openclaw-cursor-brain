...
| `CURSOR_PROXY_REQUEST_TIMEOUT`          | `300000` | Per-request timeout (ms, 5 min). Values &lt; 60000 or invalid are clamped to 60000 or 300000. |
| `CURSOR_PROXY_DEGRADED_TIMEOUT`         | `300000` | Timeout (ms) when already degraded (after failures/timeouts). Same clamping as above.         |
| `CURSOR_PROXY_MAX_CONSECUTIVE_FAILURES` | `8`      | Consecutive failure threshold; proxy self-exits for restart when exceeded                     |
| `CURSOR_PROXY_MAX_CONSECUTIVE_TIMEOUTS` | `5`      | Consecutive timeout threshold; proxy self-exits for restart when exceeded                     |
| `CURSOR_PROXY_STREAM_RESOLVE_GRACE_MS`  | `5000`   | After killing child, wait this long for stdout to close before returning 503                  |
